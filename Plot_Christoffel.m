...
for n=N_max:N_max
    ...
    for l=1:T_tries
        ...
        A=exp(2*pi*1i*x*k);
        T=A'*diag(c)*A;
        Trho=randn(size(T))+1i*randn(size(T));
...
colorbar
hold on
stem3(eps_op,T_values,max(max(h))*ones(size(eps_op)),'rx','MarkerSize',16)
hold on
stem3(eps,max(4.14-eps,0),max(max(h))*ones(size(eps)),'m.','MarkerSize',16)
